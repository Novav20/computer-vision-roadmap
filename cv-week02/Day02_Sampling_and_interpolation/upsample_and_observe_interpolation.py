...
SOURCE_IMAGE_PATH = "crono_ds_x8.png" # Path to the downsampled image
UPSAMPLE_FACTOR = 4 # How much to magnify the small image
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "upsampled_images")
...
if __name__ == "__main__":
    ...
    cv2.imwrite(os.path.join(OUTPUT_DIR, f"upsampled_nearest_neighbor_x{UPSAMPLE_FACTOR}.png"), upsampled_nn)
    ...
    cv2.imwrite(os.path.join(OUTPUT_DIR, f"upsampled_bilinear_x{UPSAMPLE_FACTOR}.png"), upsampled_bilinear)
    print(f"Bilinear result: {upsampled_bilinear.shape[1]}x{upsampled_bilinear.shape[0]}")
    ...
    cv2.imwrite(os.path.join(OUTPUT_DIR, f"upsampled_bicubic_x{UPSAMPLE_FACTOR}.png"), upsampled_bicubic)
    ...
    print("- Nearest Neighbor: Blocky appearance, 'pixelated' look. Preserves sharp edges but creates visible blocks.")
    print("- Bilinear: Smoother transitions, less blocky, but can blur sharp edges.")
    print("- Bicubic (if shown): Often a good compromise, sharper than bilinear but less blocky than nearest.")